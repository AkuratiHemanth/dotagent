...
gen_command = pp.Suppress(pp.Combine("{{gen" + pp.SkipTo("}}") + "}}"))
gen_command.set_name("gen_command")

code_chunk <<= code_chunk_no_infix | gen_command | infix_operator_block
code_chunk.set_name("code_chunk")


assistant_tag_start = pp.Suppress("{{#assistant}}")
assistant_tag_end = pp.Suppress("{{/assistant}}")

assistant_segment = pp.Group(assistant_tag_start + gen_command + assistant_tag_end)
single_command_rule = pp.Forward()
single_command_rule <<= assistant_segment | (single_command_rule + pp.Suppress(pp.OneOrMore(assistant_segment)))
single_command_rule.set_name("single_command_rule")

ending_command = pp.Suppress("{{#assistant}}{{gen 'write' }}{{/assistant}}")

program <<= program_chunk + pp.Optional(ending_command) + pp.StringEnd()
